...
test.describe('Automated Test Submission - CHASIDE', () => {
    ...
    test('Fill out test dynamically', async ({ page }) => {
        console.log('Iniciando test automatizado de CHASIDE...');
        ...
        console.log('Navegando hacia /chaside...');
        await page.goto('http://localhost:5173/chaside');
        ...
        console.log('Iniciando el ciclo de escaneo y respuestas para CHASIDE...');
        while (!isFinished && loopCount < 100) {
            ...
            // Respondemos si hay campos de texto abiertos en CHASIDE
            const textInputs = await page.locator('input[type="text"]:visible, textarea:visible').all();
            for (const input of textInputs) {
                try {
                    const val = await input.inputValue();
                    if (!val || val.trim() === '') {
                        await input.fill('Respuesta automática CHASIDE Playwright');
                    }
                } catch (e) { }
            }
            ...
            if (canSubmit) {
                ...
                if (page.url().includes('client')) {
                    isFinished = true;
                    console.log('Redireccionado al inicio. TEST CHASIDE TERMINADO.');
                    break;
                } else {
        ...
        console.log('Éxito completo. El test de CHASIDE regresó exitosamente al panel Cliente.');
    });
});
